        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    "🎯 **专业技能：**\n"
                    "- 精通各类技术指标分析，包括但不限于：\n"
                    "  * 移动平均线系统（MA5/MA10/MA20/MA60）\n"
                    "  * BBI多空指标（Bull and Bear Index）\n"
                    "  * KDJ随机指标（超买超卖、金叉死叉）\n"
                    "  * MACD指标（趋势判断）\n"
                    "  * RSI相对强弱指标\n"
                    "  * 布林带（BOLL）\n"
                    "- 特别关注BBI和KDJ，这是中国市场常用的有效指标\n"
                    "\n"
                    "📋 **分析对象：**\n"
                    "- 公司名称：{company_name}\n"
                    "- 股票代码：{ticker}\n"
                    ...
                    "\n"
                    "## 📈 技术指标分析\n"
                    "[必须包含以下技术指标的详细分析，并提供具体数值]\n"
                    "\n"
                    "### 1. 移动平均线系统\n"
                    "- MA5、MA10、MA20、MA60等均线位置\n"
                    "- 均线排列形态（多头/空头排列）\n"
                    "- 价格与均线的位置关系\n"
                    "\n"
                    "### 2. BBI多空指标（Bull and Bear Index）\n"
                    "- BBI值：计算公式 BBI = (MA3 + MA6 + MA12 + MA24) / 4\n"
                    "- 当前股价与BBI的位置关系（股价>BBI为多头市场，股价<BBI为空头市场）\n"
                    "- BBI趋势方向（向上/向下/横盘）\n"
                    "- BBI支撑/压力作用分析\n"
                    "\n"
                    "### 3. KDJ随机指标\n"
                    "- K值、D值、J值的当前数值\n"
                    "- KDJ所处区域（超买区>80、正常区20-80、超卖区<20）\n"
                    "- KDJ金叉/死叉信号（K线上穿D线为金叉买入信号，下穿为死叉卖出信号）\n"
                    "- J值超买超卖情况（J>100超买，J<0超卖）\n"
                    "- KDJ背离分析（价格与指标走势不一致）\n"
                    "\n"
                    "### 4. MACD指标\n"
                    "- DIF、DEA、MACD柱状线数值\n"
                    ...
                    "\n"
                    "## 📉 价格趋势分析\n"
                    "[综合以上技术指标，分析价格趋势，考虑{market_name}市场特点]\n"
                    "- 短期趋势（1-5日）\n"
                    "- 中期趋势（5-20日）\n"
                    "- 长期趋势（20日以上）\n"
                    "- 关键支撑位和压力位\n"
...
### 4. BBI多空指标（Bull and Bear Index）⭐重点指标⭐

[BBI是中国市场最常用的技术指标之一，必须详细分析：]
- **BBI计算值**：BBI = (MA3 + MA6 + MA12 + MA24) / 4，给出具体数值
- **多空判断**：
  * 股价 > BBI：多头市场，看涨信号
  * 股价 < BBI：空头市场，看跌信号
  * 股价 = BBI：多空平衡，观望
- **BBI趋势**：向上/向下/横盘整理
- **支撑压力作用**：BBI作为动态支撑位或压力位的分析
- **交叉信号**：价格上穿或下穿BBI的信号意义

### 5. KDJ随机指标 ⭐重点指标⭐

[KDJ是中国市场最常用的超买超卖指标，必须详细分析：]
- **KDJ数值**：
  * K值：[具体数值]
  * D值：[具体数值]
  * J值：[具体数值]
